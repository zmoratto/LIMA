...
\begin{enumerate}
	\item{svn co  https://babelfish.arc.nasa.gov/svn/stereopipeline/sandbox/lima lima} - dowload ATK
	\item{cd /lima}
	\item{mkdir build}
	\item{cd build}
	\item{cmake ..} - here is big double dots 
	\item{make install} - make sure that usr/local/bin is in \$PATH. Otherwise, change CMAKE\_INSTALL\_PREFIX by command "ccmake .." in the "build" directory, then executable files will be generated in CMAKE\_INSTALL\_PREFIX/bin directory.
\end{enumerate}
...
\subsection{How to test SFM:}
\begin{enumerate}
	\item{cd /lima/build}
	\item{make test} - view the file in lima/build/Testing/Temporary/LastTest.log
\end{enumerate}
