...
{
	// generate transformation matrix from tracks: remember results to speed testing
	// vector<vector<LOLAShot> > trackPts2 = LOLAFileRead("../data/Apollo15-LOLA/RDR_1E6E_21N28NPointPerRow_csv_table.csv");
	// Matrix3x3 sol1(0.999198,0.0020664,41.7295,-0.000307165,1.00242,-47.5016, 0,0,1);
	// Matrix3x3 sol2(0.999323,-0.00175819,52.1883,-0.000745662,1.00171,-41.3955,0,0,1);
	// vector<vector<AlignedLOLAShot> > aligneda  = setup_tracks(trackPts2, "../data/Apollo15-CAM-CUB/AS15-M-1134.lev1.cub", sol1);
	// vector<vector<AlignedLOLAShot> > alignedb = setup_tracks(trackPts2, "../data/Apollo15-CAM-CUB/AS15-M-1135.lev1.cub", sol2);
	// Matrix3x3 H = find_track_homography(alignedb, aligneda);
	// cout << H << endl;
	// return 0;

	// get translation between two images
	// Matrix3x3 H = find_image_homography("../data/Apollo15-CAM-CUB/AS15-M-1134.lev1.cub", "../data/Apollo15-CAM-CUB/AS15-M-1135.lev1.cub", 2.0, 5.0, 22.0, 7.0);
	// cout << H << endl;
	// return 0;

	// code to composite two images together
	// transforms from original image information
	// // Matrix3x3 P(-0.000828107,1.07346e-07,0.997705,1.63897e-05,-0.000840201,-0.0676932,5.60053e-09,2.11604e-10,-0.000862862);
	// // Matrix3x3 Pinv(0.000863626,2.63516e-07,0.998885,1.63959e-05,0.000844465,-0.0471861, 5.61093e-09,1.80992e-10,0.000828915);
	// // transforms based on LIDAR readings
	// // Matrix3x3 P(-0.000854256, 4.98922e-06, 0.99816, 1.92002e-05, -0.000869587, -0.0606141, 6.74695e-09, 8.58494e-10, -0.000899481);
	// Matrix3x3 Pinv(0.000877609,5.85286e-06,0.998645,1.84986e-05,0.000854037,-0.0520229,6.59651e-09,8.1404e-10,0.000833132);
	// overlay_image("../data/Apollo15-CAM-CUB/AS15-M-1134.lev1.cub", "../data/Apollo15-CAM-CUB/AS15-M-1135.lev1.cub", Pinv);
	// return 0;
